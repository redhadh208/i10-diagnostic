...
class BluetoothService {
  ...
  // Lire temps d'injection par cylindre (PID Hyundai spécifique)
  async readInjectorTimes() {
    const out = { inj1: null, inj2: null, inj3: null, inj4: null };
    try {
      // Commandes propriétaires Hyundai pour temps d'injection
      const cmds = { inj1:'2180', inj2:'2181', inj3:'2182', inj4:'2183' };
      for (const [k,c] of Object.entries(cmds)) {
        const raw = await this.send(c);
        if (raw && !raw.includes('NO DATA')) {
          const clean = raw.replace(/\s/g,'').toUpperCase();
          const bytes = [];
          for (let i=0; i<clean.length; i+=2) bytes.push(parseInt(clean.substr(i,2),16));
          if (bytes.length >= 2) out[k] = ((bytes[0]*256)+bytes[1])/1000;
        }
  ...
  async readDTC() {
    ...
    for (const p of parts) {
      if (p === '0000') continue;
      const decoded = this._decodeDTC(p);
  ...
  _decodeDTC(code) {
    ...
    return (map[code[0].toUpperCase()] || 'P') + code.substring(1);
  }
